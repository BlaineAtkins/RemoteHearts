...
*/
  if(strcmp(topic,"KenzieLiamHeart/dualColor")==0){
    ...
    if(rcvNum==-1){ //if other heart just came online, ignore the value (-1), but ping it to let it know we're here too and give it our value
      ...
      currentColorRemote = atoi((char *)payload); 
  
      if(dualColorMode){
        if(clientNum==1){ //kenzie on the right -- but receiving so it's backwards
          for(int i=6;i<12;i++){
            lights.setPixelColor(i, lights.Color(colors[currentColorRemote][0],colors[currentColorRemote][1],colors[currentColorRemote][2]));
          }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientName)) { //name of this client
      ...
      client.subscribe("KenzieLiamHeart/dualColor");
    } else {
      statusLED(1023,0,0);
...
void confirmColorMode(){ //every day, re-publish the current color mode to the MQTT broker since it only retains the last message for 3 days
  if(millis()-confirmColorModeTimer>60000*60*24 && client.connected()){
    ...
    client.publish("KenzieLiamHeart/dualColor",tempDualColorMode,true);
