...
int clientNum=2; //1 is Blaine, 2 is Chelsey
...
void setup() {
  ...
  if(clientNum==1){ //SEARCH WiFiManager.ccp for "Starting Config Portal" BELOW THAT IS THE CODE TO CHANGE STATUS LED FROM RED TO ORANGE
    if(!wifiManager.autoConnect("Blaine Heart Setup")){
      ESP.restart();
    }
  }else{
    if(!wifiManager.autoConnect("Chelsey Heart Setup")){
      ESP.restart();
    }
  }
...
*/
  ...
  if(rcvNum==-1){ //if other heart just came online, ignore the value (-1), but ping it to let it know we're here too and give it our value
    ...
    if(clientNum==1){
      client.publish("BlaineChelseyHeart/ChelseyRcv", itoa(currentColor, sendVal,10));
    }else{client.publish("BlaineChelseyHeart/BlaineRcv", itoa(currentColor, sendVal,10));}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    char* clientName;
    if(clientNum==1){
      clientName="BlaineHeart";
    }else{clientName="ChelseyHeart";}
    if (client.connect(clientName)) { //name of this client
      Serial.println("connected");
      statusLED(0,0,1023);
      // Once connected, publish an announcement...
      if(clientNum==1){
        client.publish("BlaineChelseyHeart/ChelseyRcv", itoa(-1, sendVal,10)); // -1 indicates we just came online and are requesting other heart's value
        // ... and resubscribe
        client.subscribe("BlaineChelseyHeart/BlaineRcv");
      }else{
        client.publish("BlaineChelseyHeart/BlaineRcv", itoa(-1, sendVal,10)); 
        // ... and resubscribe
        client.subscribe("BlaineChelseyHeart/ChelseyRcv");
      }
    } else {
      statusLED(1023,0,0);
